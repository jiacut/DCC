...
def get_criterion(p):
    if p['criterion'] == 'end2end':
        ...
        criterion2 = ConfidenceBasedCE(p['confidence_threshold'], p['criterion_kwargs']['apply_class_balancing'])
        return (criterion1, criterion2)
...
def get_train_dataset(p, transform, to_augmented_dataset=False, to_neighbors_dataset=False, to_end2end_dataset=False, split=None):
    # Base dataset
...
def get_val_dataset(p, transform=None, to_neighbors_dataset=False, to_end2end_dataset=False, has_neighbor=False):
    # Base dataset
    ...
    if to_end2end_dataset:
        ...
        dataset = End2EndDataset(dataset, indices, 5, has_neighbor)
...
def get_train_transformations(p):
    if p['augmentation_strategy'] == 'standard':
    ...
    elif p['augmentation_strategy'] == 'weak':
    ...
    elif p['augmentation_strategy'] == 'strong':
        return transforms.Compose([
            transforms.RandomResizedCrop(**p['augmentation_kwargs']['random_resized_crop']),
            transforms.RandomHorizontalFlip(),
            transforms.RandomApply([
            ...
            transforms.RandomGrayscale(**p['augmentation_kwargs']['random_grayscale']),
            RandAugment(p['strong_augmentation_kwargs']['num_strong_augs'], p['strong_augmentation_kwargs']['prob_strong_augs']),
            transforms.ToTensor(),
            transforms.Normalize(**p['augmentation_kwargs']['normalize'])
        ])
    
    elif p['augmentation_strategy'] == 'both':
        ...
        return dict(weak=weak_transform, strong=strong_transform)
    else:
...
def adjust_beta(p, epoch):
    beta = p['beta']
    epoch = epoch - p['init_epochs']

    if epoch > 0 and epoch <= p['warmup_epochs']:
        beta = 0.5 * (1.0 - math.cos(math.pi * epoch / p['warmup_epochs'])) * beta
