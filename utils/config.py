...
def create_config(config_file_env, config_file_exp):
    ...
    if cfg['setup'] in ['scan', 'selflabel']:
        ...
        cfg['topk_neighbors_val_path'] = os.path.join(selflabel_dir, 'topk-val-neighbors.npy')
        cfg['log_output_file'] = os.path.join(selflabel_dir, 'log.txt')
        cfg['features'] = os.path.join(selflabel_dir, 'features')
