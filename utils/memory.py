...
class MemoryBank(object):
    ...
    def mine_nearest_neighbors(self, topk, model, calculate_accuracy=True):
        ...
        if calculate_accuracy:
            ...
            detail_acc = []
            for tk in topks:
                accuracy = np.mean(neighbor_targets[:, :tk] == anchor_targets[:, :tk])
                detail_acc.append(accuracy)

            return indices[:, 1:], accuracy, detail_acc
        else:
            return indices[:, 1:]
