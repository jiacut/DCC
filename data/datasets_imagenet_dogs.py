...
class ImageNetDogs(Dataset):
    ...
    def __loadfile(self, data_file, labels_file=None):
        data_path = os.path.join(self.root, 'data.npy')
        datas = np.load(data_path)
        datas = datas.tolist() # array to list

        label_path = os.path.join(self.root, 'label.npy')
        labels = np.load(label_path)
        labels = labels.tolist() # array to list
        '''
        datas = []
        labels = []
