...
class End2EndModel(nn.Module):
    def __init__(self, backbone, head='mlp', features_dim=128, nheads=1, nclusters=10):
        ...
        if head == 'linear':
            ...
            self.contrastive_head = nn.Sequential(nn.Linear(self.backbone_dim, self.backbone_dim),nn.ReLU(), nn.Linear(self.backbone_dim, features_dim))
        else:
            raise ValueError('Invalid head {}'.format(head))
...
class ClusteringModel(nn.Module):
    def __init__(self, backbone, nclusters, nheads=1):
        ...
        assert(self.nheads > 0)
        self.cluster_head = nn.ModuleList([nn.Linear(self.backbone_dim, nclusters) for _ in range(self.nheads)])
        #self.cluster_head = nn.ModuleList([nn.Sequential(
                #nn.Linear(self.backbone_dim, self.backbone_dim),
                #nn.ReLU(),
                #nn.Linear(self.backbone_dim, nclusters)) for _ in range(self.nheads)])
